...
const REPLIES: Record<string, { lines: string[]; tone?: "error" | "success" }> = {
  ...
  sudo: {
    lines: ["dev no está en el archivo sudoers. Este intento será documentado."],
    tone: "error"
  },
  rm: {
    lines: ["operación bloqueada: este segundo cerebro sí hace copias de seguridad."],
    tone: "error"
  }
}
...
export const secretosCommands: CommandMap = {
  ...
  matrix: {
    run: (ctx) => {
      ...
      ctx.setEffect("none")
      ctx.print(
        ["wake up, dev…", "the docs have you.", "follow the white rabbit: /cat"],
        "success"
      )
    }
  },
  ...
  party: {
    run: (ctx) => {
      const theme = pickRandom(TERMINAL_THEMES.filter((item) => item !== ctx.theme))
      ctx.setTheme(theme)
      ctx.setEffect("rainbow")
      ctx.print([`♪ tema ${theme} desbloqueado · /rainbow para detener ♪`], "success")
    }
  },
  ...
  rainbow: {
    run: (ctx) => {
      ...
      ctx.setEffect(next)
      ctx.print([`rainbow mode: ${next === "rainbow" ? "on" : "off"}`], "success")
    }
  }
}
