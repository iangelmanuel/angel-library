...
export default function TagExplorer({ tags }: { tags: TagItem[] }) {
  ...
  const visibleTags = useMemo(() => {
    ...
    const filtered = normalizedQuery
      ? tags.filter(({ tag }) => tag.toLocaleLowerCase('es').includes(normalizedQuery))
      : [...tags];
  ...
  return (
    <div className="tag-explorer">
      <div className="terminal-toolbar">
        <span className="terminal-toolbar__prompt" aria-hidden="true">
          $
        </span>
        ...
        <input
          ...
          autoComplete="off"
        />
        <span className="terminal-toolbar__count" aria-hidden="true">
      ...
      {visibleTags.length > 0 ? (
        <div className="tag-grid">
          {visibleTags.map(({ tag, count }) => (
            ...
            >
              ...
              <span className="tag__count" aria-label={`${count} entradas`}>
                {count}
              </span>
            </a>
          ))}
        ...
        <p className="terminal-empty">
          <span aria-hidden="true">exit 1:</span> no hay tags que coincidan con “{query}”
        </p>
      )}
    </div>
  );
