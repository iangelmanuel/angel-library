...
Un archivo `.ts` dentro de `src/pages/` con un `export const GET` es una ruta de API de Astro. Consulta [Endpoints (rutas de API)](/frontend/astro/astro-endpoints) para el mecanismo general. Aquí sirve para generar el manifiesto dinámicamente en vez de un JSON estático en `public/`, por lo que también puede leer `SITE`.
