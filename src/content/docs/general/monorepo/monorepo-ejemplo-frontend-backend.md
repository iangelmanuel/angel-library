...
Este es exactamente el caso de uso real que mencionaba [la guía de qué es un monorepo](/general/monorepo/monorepo-que-es): un tipo que cambia en `shared-types` rompe la build de `api` o de `web` de inmediato si dejan de coincidir, en vez de descubrirse en producción cuando el frontend recibe un campo que ya no existe.
