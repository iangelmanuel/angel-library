...
Las convenciones — verbo HTTP correcto, cómo paginar/filtrar, formato de errores — son las mismas que en [REST y CRUD](/backend/express/express-rest-crud) y [Paginación, filtrado y búsqueda](/backend/express/express-api-paginacion). Esta guía es solo la sintaxis de Route Handlers, que ya está introducida en [Endpoints (Route Handlers)](/frontend/nextjs/nextjs-endpoints).
...
`params` como Promise (no un objeto directo) es un cambio de Next.js 15+ — ver [Leer params y searchParams](/frontend/nextjs/nextjs-params-searchparams) para el detalle completo de ese cambio.

## La diferencia principal frente a Express: sin middleware por ruta

Igual que en Astro, Next.js no tiene una cadena de middlewares específica por Route Handler — la protección amplia va en `proxy.ts` (ver [Backend en Next.js](/backend/nextjs/nextjs-backend-arquitectura)), y chequeos específicos de la ruta (como `auth()` en el ejemplo de `POST` arriba) van al principio del handler mismo.
...
- Un Route Handler puede declarar `export const runtime = 'edge'` para correr en el Edge Runtime (más rápido para arrancar, con limitaciones — por ejemplo, Prisma necesita un driver adapter especial ahí, ver [Prisma en Next.js](/backend/nextjs/nextjs-prisma)) — el default (`'nodejs'`) alcanza para la gran mayoría de los casos.
