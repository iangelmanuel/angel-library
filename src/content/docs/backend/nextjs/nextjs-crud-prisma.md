...
`revalidatePath` es lo que hace que la Server Component de listado (`/posts`) muestre el dato actualizado en la siguiente navegación/render, sin necesitar un `fetch` manual desde el cliente ni recargar toda la página — ver [Revalidación de paths](/frontend/nextjs/nextjs-revalidate-path) para el mecanismo completo.
...
- Este mismo CRUD también podría exponerse completo como Route Handlers (`POST`/`PATCH`/`DELETE` en `app/api/posts/route.ts`) si algo externo a la propia app necesita consumirlo — ver [API REST con Route Handlers](/backend/nextjs/nextjs-api-rest).
