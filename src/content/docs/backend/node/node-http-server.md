...
- Esto **no tiene** `next()` — ese concepto (una cadena de funciones middleware que se pasan el control entre sí) no existe en el `http` nativo, lo agregan frameworks como Express. Ver [Middlewares en Express](/backend/express/express-middlewares).
