...
1. **Mapeo ruta → archivo**: `url.pathname` se concatena a la carpeta pública con `path.join` (no con `+` a mano, ver [Filesystem](/backend/node/node-filesystem)) para que funcione igual en Windows y Linux.
