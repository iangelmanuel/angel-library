...
- Este patrón (rol único por usuario, `req.user.rol === 'admin'`) alcanza para la mayoría de los casos; sistemas con permisos más finos (varios roles por usuario, permisos por recurso específico) necesitan un modelo más elaborado — ver [Roles y permisos](/backend/express/express-roles-permisos).
