...
Esta versión firma un token para cualquier `userId` que le pasen, sin verificar contraseña — es intencional, para aislar el mecanismo de JWT antes de sumar autenticación real. El paso siguiente es [Auth completa](/backend/express/express-auth-completa), que agrega registro, login con bcrypt, y cookies en vez de headers manuales.

## Consideraciones

- `SECRET` hardcodeado en el código es **solo para este ejemplo aislado** — en cualquier proyecto real va en una variable de entorno, ver [Variables de entorno en Node](/backend/node/node-env-vars).
- `(req as any).userId` es un atajo rápido para esta demo mínima — la forma correcta de tipar `req.user` está en [Middleware de autenticación](/backend/express/express-auth-middleware).
