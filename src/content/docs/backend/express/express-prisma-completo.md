...
Esta versión no valida el body ni maneja errores — el siguiente paso natural es aplicar el patrón completo de [CRUD completo](/backend/express/express-crud-completo) (validación con Zod, status codes correctos, error handler) sobre esta misma base.

## Consideraciones

- `npx prisma migrate dev` **no** es el comando de producción — en un deploy real se usa `npx prisma migrate deploy`, que aplica migraciones existentes sin generar nuevas interactivamente (ver [Prisma en Express](/backend/express/express-prisma)).
