...
```js
const url = new URL(
  "/products/keyboard?sort=price#reviews",
  "https://example.com"
)
...
```js
new URL("../image.png", "https://example.com/docs/guide/").toString()
...
Al enviar `FormData` con Fetch, no escribas manualmente el header `Content-Type`; el runtime añade también el límite (_boundary_) correcto.

## `Blob`, `File`, `ArrayBuffer` y typed arrays

Esta sección conecta archivos del navegador con memoria binaria. Para vistas, endianness, transferencias, `SharedArrayBuffer` y `Atomics`, consulta [Datos binarios y typed arrays](/languages/javascript/javascript-binary-data).
