...
technologies: [git/git/git-undo-commit-keep-changes]
...
- `reset --hard` no es "peligroso" en el sentido de romper el repo: el commit sigue existiendo un tiempo en el [reflog](/git/git/git-reflog), recuperable con `git reset --hard <hash-anterior>`. Pero no confíes en eso como plan A — el reflog tiene una expiración.
