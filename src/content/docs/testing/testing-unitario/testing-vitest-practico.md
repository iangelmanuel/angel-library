...
it.each([
  ...
  "calcula $expected para $subtotal con tasa $rate",
...
`vi.spyOn` conserva la implementación hasta que la reemplazas; `vi.fn` crea una función controlada. `vi.mock` sustituye un módulo completo y se eleva —_hoisting_— durante la transformación, por lo que conviene usarlo solo en fronteras claras y restaurar estado entre casos.
