...
- `docker exec` no funciona en un contenedor detenido — hace falta que esté `Running` (ver [Ciclo de vida](/devops/docker-contenedores/docker-lifecycle)).
- Cualquier cambio hecho a mano dentro de una sesión `exec` (instalar algo o editar un archivo) se pierde si el contenedor se elimina y no se refleja en la imagen. Los cambios permanentes deben declararse en el Dockerfile.
