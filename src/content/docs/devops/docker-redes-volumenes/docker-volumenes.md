...
El filesystem de un contenedor vive en su capa de escritura (ver [Arquitectura](/devops/docker-conceptos/docker-arquitectura)) — se pierde para siempre si el contenedor se borra. Para datos que necesitan sobrevivir (una base de datos, uploads de usuarios), Docker ofrece tres mecanismos de almacenamiento por fuera de esa capa.
...
- Con un volumen nombrado, si el contenedor se borra pero el volumen no, un contenedor **nuevo** montado sobre ese mismo volumen ve los mismos datos — ver [Persistencia de datos](/devops/docker-redes-volumenes/docker-persistencia-datos) para el detalle completo de este flujo.
