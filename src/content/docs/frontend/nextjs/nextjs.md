...
Comienza por [Primer proyecto](/frontend/nextjs/nextjs-getting-started), [Estructura y configuración](/frontend/nextjs/nextjs-project-structure-configuration) y [Fundamentos de routing](/frontend/nextjs/nextjs-routing-fundamentals). Aquí debes poder responder qué URL genera un archivo, qué conserva un layout y cuándo un componente se ejecuta en servidor.

### Etapa 2 — Comprender el renderizado

Continúa con [Server y Client Components](/frontend/nextjs/nextjs-server-client-components), [Directivas](/frontend/nextjs/nextjs-directivas) y [loading/error/not-found](/frontend/nextjs/nextjs-page-error-loading). Después estudia [streaming y Suspense](/frontend/nextjs/nextjs-streaming-suspense). El objetivo no es memorizar directivas: es dibujar la frontera entre trabajo del servidor, JavaScript del navegador y contenido que puede llegar progresivamente.

### Etapa 3 — Modelar rutas y navegación

Profundiza en layouts, grupos, parámetros, `<Link>`, hooks de navegación, redirecciones y generación estática. Cuando lo anterior sea predecible, las [rutas paralelas e interceptadas](/frontend/nextjs/nextjs-parallel-intercepting-routes) tendrán un caso de uso claro en lugar de parecer reglas aisladas.

### Etapa 4 — Datos y caché

Aprende primero dónde consultar datos y qué significa estático, dinámico y cacheado. Luego estudia el modelo clásico de `fetch` y el modelo explícito de [Cache Components](/frontend/nextjs/nextjs-cache-components). Termina con invalidación por ruta o etiqueta. Una aplicación puede usar datos correctos y aun así mostrar información obsoleta si no define cuándo deja de ser válida.
