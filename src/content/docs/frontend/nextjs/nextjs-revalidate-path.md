...
`revalidate` (el de [fetching con revalidate](/frontend/nextjs/nextjs-fetching-revalidate)) refresca por tiempo. `revalidatePath` refresca **al instante, bajo demanda** — el caso típico es "acabo de guardar algo, quiero que la página que lo lista se actualice ya, no en N segundos".
