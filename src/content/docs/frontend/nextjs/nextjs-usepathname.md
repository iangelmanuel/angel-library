...
- Si el proyecto usa `rewrites` (en `next.config`) o [Proxy](/frontend/nextjs/nextjs-proxy), `usePathname()` puede devolver la ruta reescrita (destino), no la que el usuario ve en la barra de direcciones — puede causar un mismatch de hidratación si el componente depende de eso en el primer render.
