...
- `<Link>` **siempre** es la opción por defecto para navegar — [`useRouter`](/frontend/nextjs/nextjs-userouter) queda para navegación programática real (después de un submit, un timeout), no como reemplazo de un link normal.
...
- Con [Proxy](/frontend/nextjs/nextjs-proxy) haciendo un rewrite, el prefetch necesita que le digas tanto la URL a mostrar como la URL real a precargar (prop `as`) — si no, Next no sabe qué precargar de antemano.
