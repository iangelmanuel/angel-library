...
Cómo se ve y se comporta el scroll, no cómo animar según él — para eso ver [Animaciones CSS](/snippets/css-animations).
...
- `scroll-behavior: smooth` debería ir siempre junto al bloque de `prefers-reduced-motion` que fuerza `scroll-behavior: auto` — ver [CSS Reset](/snippets/css-reset), que ya lo incluye.
