...
Este documento es la versión conceptual. Para ver MVC aplicado en código real, con estructura de carpetas, flujo de request y el error handler central, ver [Estructura MVC para APIs Express](/patterns/backend-mvc-structure) — la implementación completa para un backend Express (`routes` → `controllers` → `services` → `repositories`, donde `controllers` + `services` juntos cubren el rol de Controller y `services`/`repositories` el de Model).
