...
Cada vez que un componente necesita "recordar" algo entre renders y que la UI reaccione cuando cambia, es `useState`. Es la base sobre la que se explican todos los demás hooks de estado (`useReducer`, `useContext` para compartirlo, `useRef` para lo que _no_ necesita re-renderizar).
