...
- **`rootMargin` es el ajuste más útil para lazy loading**: un valor positivo (`'200px'`) hace que `isIntersecting` se vuelva `true` _antes_ de que el elemento sea literalmente visible, dando tiempo a que la imagen o los datos carguen mientras el usuario todavía está scrolleando hacia ahí.
