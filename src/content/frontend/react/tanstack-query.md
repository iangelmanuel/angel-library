...
- No reemplaza a [Zustand](/frontend/react/zustand) ni a Context: Query es específicamente para estado que **viene de un servidor** (con cache, refetch, invalidación). Estado que es puramente del cliente (un modal abierto, un tema) no necesita nada de esto.
