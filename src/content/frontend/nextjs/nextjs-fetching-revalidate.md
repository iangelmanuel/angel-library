...
Esto describe el modelo de caché "clásico" de Next (el que tiene la mayoría de los proyectos hoy). Next 16 introdujo un modelo nuevo opt-in ("Cache Components", con la directiva `use cache` — ver [Directivas](/frontend/nextjs/nextjs-directivas)) que reemplaza esto a futuro, pero mientras no actives `cacheComponents: true` en `next.config`, es este modelo el que aplica.
