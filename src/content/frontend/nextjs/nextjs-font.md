...
Igual espíritu que [Fontsource en Astro](/languages/css/css-fonts): nada de un `<link>` a Google Fonts que dispara una request externa. `next/font` descarga el archivo en build y lo self-hostea junto al resto de los assets estáticos — cero requests a Google desde el navegador del usuario, y sin el parpadeo de layout shift que causa cargar una fuente de forma tardía.
