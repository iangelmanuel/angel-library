...
Los botones que solo muestran un icono necesitan un nombre (`aria-label` o texto visualmente oculto). El nombre debe describir la acción —“Cerrar diálogo”— y no la apariencia —“Icono X”—. La guía de [atributos ARIA](/accessibility/a11y-interaccion/accessibility-aria-attributes) explica cómo elegir entre `aria-label`, `aria-labelledby`, `aria-describedby` y los estados más frecuentes. Prueba el árbol de accesibilidad del navegador: si nombre, rol o estado son confusos, el componente también lo será para tecnologías asistivas.
