...
`fetch` es siempre seguro: solo actualiza tu conocimiento de "qué hay en el remoto" (`origin/main`), sin mezclar nada con tu trabajo. `pull` da un paso más: intenta integrar esos cambios nuevos a tu rama actual ahí mismo. Si prefieres revisar qué llegó antes de mezclarlo, `git fetch` + `git log origin/main` + `git merge origin/main` (a mano) da el mismo resultado que `pull`, pero con un chequeo en el medio.

## `pull` con rebase en vez de merge

Por defecto, `git pull` mergea. Muchos equipos prefieren que rebasee en su lugar, para mantener el historial lineal (ver [Merge vs rebase](/git/git/git-merge-vs-rebase)):
