...
Mismo criterio que el [skill de SEO para Astro](/skills/ia-skills/skill-seo-astro): carpeta completa, no un prompt directamente línea. `SKILL.md` liviano más `references/` con el código real de [SEO completo en Next.js](/seo/nextjs/nextjs-seo-completo) — `buildMetadata()`, los 5 builders de JSON-LD, el title template nativo, todo.
...
seo: {
/** Default title — used as-is on the home page, and as the " — {name}" suffix elsewhere. _/
title: "Acme — Software, marca y comunicación para empresas",
/_* Next applies this template to any child page that only sets its own `title`. */
...
/** Short page title — Next applies the root layout's template automatically. _/
title?: string;
/_* Already-built title, bypassing the template — for pages that shouldn't carry the brand suffix. _/
fullTitle?: string;
description?: string;
/_* Relative path — "/servicios", "/blog/mi-post". _/
path?: string;
image?: string;
imageAlt?: string;
noindex?: boolean;
...
section?: string;
tags?: string[];
/_* Extra keywords — added to SITE.seo.keywords, not a replacement. */
keywords?: string[];
}
...
/** Generates /robots.txt. With SITE.seo.noindex on (staging/preview), blocks the entire site. _/
export default function robots(): MetadataRoute.Robots {
if (SITE.seo.noindex) {
return { rules: { userAgent: "_", disallow: "/" } };
}
...
- Los 4 archivos de `references/` son el mismo código de [SEO completo en Next.js](/seo/nextjs/nextjs-seo-completo), campo por campo — la receta es la versión para leer y entender; esta skill es la misma información empaquetada para que un agente la ejecute.
