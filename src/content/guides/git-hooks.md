...
Cada repo creado con `git init` (o `clone`) ya trae una carpeta `.git/hooks/` con ejemplos `.sample` de cada hook disponible — no hacen nada hasta que les sacás el `.sample` del nombre y los hacés ejecutables.
