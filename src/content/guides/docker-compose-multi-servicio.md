...
related: [guides/docker-compose-basico, guides/docker-redes]
...
Cada servicio es alcanzable por los demás usando su **nombre de servicio** como hostname — `app` se conecta a Postgres con `db:5432` y a Redis con `cache:6379`, sin IPs ni configuración de red manual (mismo mecanismo que [Redes](/guides/docker-redes), automático en Compose).
...
- Este mismo patrón (`db` con volumen + healthcheck) es la base de la sección dedicada a [Postgres con Compose](/guides/docker-postgres-compose).
