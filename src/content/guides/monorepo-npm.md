...
npm no tiene forma nativa de correr dos scripts de `dev` que no terminan al mismo tiempo — para eso hace falta `concurrently`. El [ejemplo completo de frontend + backend](/guides/monorepo-ejemplo-frontend-backend#npm--necesita-concurrently) muestra exactamente cómo, con un backend Express y un frontend Vite reales.
