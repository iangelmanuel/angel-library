...
category: devops
stack: docker-bases-datos
order: 3
tags: [docker, postgres, psql, base-de-datos]
scope: conectar a Postgres
related: [guides/docker-postgres-run, guides/docker-exec-logs]
...
(mismo mecanismo explicado en [docker exec](/guides/docker-exec-logs) — entra a una sesión dentro del contenedor ya corriendo).
...
- Desde otro contenedor en la misma red de Docker/Compose: el **nombre del servicio** en vez de `localhost` — `postgres://user:secreto@db:5432/miapp` (ver [Redes](/guides/docker-redes) y [Postgres con Compose](/guides/docker-postgres-compose)).
...
(ver el detalle completo de por qué esto pasa en [Persistencia de datos](/guides/docker-persistencia-datos))
...
- Un `ECONNREFUSED`/timeout al conectar desde la app justo después de levantar Compose suele ser que Postgres todavía no terminó de arrancar — ver `healthcheck` + `depends_on: condition: service_healthy` en [Compose multi-servicio](/guides/docker-compose-multi-servicio).
