...
related: [guides/docker-postgres-run, guides/docker-compose-variables]
updatedAt: 2026-08-17
---

## Por qué Compose en vez del `docker run` largo

El comando de [Levantar Postgres con docker run](/guides/docker-postgres-run) funciona, pero hay que recordarlo (o guardarlo en algún lado) cada vez. Con Compose, la definición vive versionada junto al proyecto — cualquiera que clona el repo levanta la misma base de datos con un solo comando.
...
(ver [Variables con Compose](/guides/docker-compose-variables) para por qué el `.env` va aparte y en `.gitignore`)
...
Notar `@db:5432` (no `@localhost:5432`) — desde adentro de otro contenedor en la misma red de Compose, el hostname es el nombre del servicio, no `localhost` (ver [Redes](/guides/docker-redes)). `localhost:5432` sí es correcto al conectarse desde **afuera** de Docker (un cliente en tu máquina — ver la próxima guía).
