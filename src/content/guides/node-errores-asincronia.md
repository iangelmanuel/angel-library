...
async function loadUser(id: string) {
  ...
  if (!response.ok) throw new HttpError(response.status, 'No se pudo cargar el usuario');
...
async function search(query: string, signal: AbortSignal) {
  return fetch(`https://example.test/search?q=${encodeURIComponent(query)}`, { signal });
}
