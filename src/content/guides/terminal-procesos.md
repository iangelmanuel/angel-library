...
`pkill` mata **todos** los procesos que matcheen el nombre — si tenés tres instancias de `node` corriendo, las tres mueren. Cuando se necesita matar una sola, conviene ir por PID.
...
- En Windows, muchos procesos de servidores de desarrollo (Node, Vite, etc.) quedan "colgados" tras un `Ctrl+C` mal cortado — `Stop-Process -Name node -Force` es la forma rápida de limpiar todos de una.
