...
Un asistente interactivo pregunta el nombre de la carpeta, si empezar desde una plantilla y si instalar dependencias. No requiere instalar nada global de antemano.
...
Regenera los tipos TypeScript de `astro:content` (en `.astro/types.d.ts`) a partir de `src/content.config.ts`. Hace falta correrlo después de cambiar el schema de una colección para que los tipos dejen de estar desactualizados.
