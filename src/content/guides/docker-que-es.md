...
Ese contenedor se crea a partir de una **imagen**: una plantilla de solo lectura que define qué hay adentro (sistema base, dependencias, código, comando de arranque). Una imagen es a un contenedor lo que una clase es a una instancia — la imagen es el molde, el contenedor es el proceso corriendo a partir de ese molde.
