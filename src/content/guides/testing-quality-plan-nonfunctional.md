...
```ts
test('una persona recupera acceso con un enlace vigente', {
  tag: ['@smoke', '@auth'],
}, async ({ page }) => {
  // Flujo observable.
});
...
Un **SLO** (*Service Level Objective* u objetivo de nivel de servicio) expresa una meta medible del sistema. Una prueba de carga puede verificar capacidad antes de publicar; la observabilidad confirma comportamiento bajo tráfico real.
