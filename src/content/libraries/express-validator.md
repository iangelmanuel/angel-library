...
- Si el proyecto ya usa [Zod](/libraries/zod) (por ejemplo, para validar `.env` o compartir schemas con un frontend), duplicar reglas de validación en dos sistemas distintos (Zod en un lado, express-validator en otro) es trabajo repetido — un middleware genérico que valide `req.body` contra un `z.object()` cubre lo mismo sin una segunda API que aprender.
