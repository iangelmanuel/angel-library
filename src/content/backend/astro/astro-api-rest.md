...
Las convenciones — qué verbo HTTP usar, cómo paginar/filtrar, el formato de errores — son las mismas que en [REST y CRUD](/backend/express/express-rest-crud) y [Paginación, filtrado y búsqueda](/backend/express/express-api-paginacion). Esta guía es solo cómo se ven esos mismos principios en la sintaxis de endpoints de Astro, que no tiene un router de terceros (Express) sino archivos con `export function GET/POST/...`.
...
Express permite `app.get(ruta, mw1, mw2, handler)` — una cadena de middlewares específica de esa ruta. Astro no tiene ese mecanismo por archivo; la protección (auth, rate limiting) se resuelve en el [middleware global](/backend/astro/astro-backend-arquitectura) (que corre para toda request y decide qué hacer según la ruta) o a mano, al principio de cada handler, como en el ejemplo de `POST` arriba (`if (!locals.user) return 401`).
