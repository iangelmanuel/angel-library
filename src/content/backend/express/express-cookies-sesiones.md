...
En vez de devolver el [JWT](/backend/express/express-jwt) en el body de la respuesta (donde JavaScript del cliente lo tendría que guardar en `localStorage`), se manda como cookie:
...
- En aplicaciones full-stack del mismo origen, [Better Auth](/backend/express/express-better-auth) y [Auth.js](/backend/express/express-auth-js) pueden implementar estas decisiones con opciones seguras configurables. Aun así, conviene comprender el modelo para revisar sus cookies y expiración.
