...
Si el login lo maneja Supabase directamente (en vez de JWT manual, ver [JWT en Express](/backend/express/express-jwt), o [better-auth](/backend/express/express-better-auth)/[Auth.js](/backend/express/express-auth-js)):
...
- `SUPABASE_SERVICE_ROLE_KEY` es tan sensible como cualquier secreto de base de datos — ver [Variables de entorno en Node](/backend/node/node-env-vars).
- No es mutuamente excluyente con [Prisma](/backend/express/express-prisma) — es común usar Prisma para queries complejas con relaciones y tipos generados, y el client de Supabase solo para lo que Prisma no cubre (auth, storage, realtime). Ambos apuntan a la misma base Postgres.
