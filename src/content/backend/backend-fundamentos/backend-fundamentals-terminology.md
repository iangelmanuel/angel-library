...
1. [Diseño de APIs](/backend/backend-fundamentos/backend-api-design): contratos y fronteras.
2. [Idempotencia y caché](/backend/backend-fundamentos/backend-idempotencia-cache): repetición segura y frescura.
3. [Resiliencia](/backend/backend-fundamentos/backend-resiliencia-dependencias): dependencias lentas o caídas.
4. [Colas y jobs](/backend/backend-fundamentos/backend-colas-jobs): trabajo fuera de la request.
5. [Webhooks y tiempo real](/backend/backend-fundamentos/backend-webhooks-tiempo-real): comunicación entre sistemas.
6. [Archivos y object storage](/backend/backend-fundamentos/backend-archivos-object-storage): datos grandes y no estructurados.
