...
Un `Dockerfile` es una receta de texto plano: una lista de instrucciones que Docker ejecuta en orden para construir una imagen. Cada instrucción agrega una capa (ver [Capas y cache](/devops/docker-imagenes/docker-capas-cache)).
...
- El orden de las instrucciones importa para el cache de build — ver [Capas y cache](/devops/docker-imagenes/docker-capas-cache) antes de escribir un Dockerfile "de verdad".
- Un Dockerfile completo de producción normalmente usa multi-stage build (compilar en una etapa, correr en otra más liviana) — ver [Multi-stage builds](/devops/docker-imagenes/docker-multi-stage).
