...
Un `.dockerignore` bien hecho también ayuda al cache (ver [Capas y cache](/devops/docker-imagenes/docker-capas-cache)): si `node_modules` o archivos de log cambian todo el tiempo pero están excluidos, no invalidan capas de `COPY . .` innecesariamente.

## Consideraciones

- Sin `.dockerignore`, es fácil terminar con secretos (`.env`, claves privadas) empaquetados dentro de la imagen. Cualquiera con acceso a ella —por ejemplo, desde un registro— podría extraerlos, aunque el Dockerfile nunca los use.
