...
El script clona el repo en `~/.nvm` y agrega las líneas necesarias a `.bashrc` / `.zshrc` (ver [bash y zsh](/terminal/terminal/terminal-linux-cli)). Después de instalar, cerrar y volver a abrir la terminal (o `source ~/.zshrc`).
