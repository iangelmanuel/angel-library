...
- **La terminal es la interfaz.** Buscar y navegar se hace por comandos (Ctrl/Cmd+K, `/search`), no por menús de blog.
...
- Autoría por archivos: se añade una entrada creando un `.md` en `src/content/<categoría>/<subcategoría>/`. La carpeta decide la categoría; mover el archivo lo recategoriza.
...
- 718 entradas en 23 categorías y 131 subcarpetas; 1719 páginas generadas.
- Tipos editoriales en uso: guides (414), resources (91), skills (80), libraries (40), recipes (19), patterns (19), utilities (12), technologies (9), practices (9), hooks (9), snippets (7), commands (4), tricks (3), integrations (2).
- `src/config/` es la fuente única: `site.ts` (identidad y SEO), `categories.ts`, `subcategories.ts`, `content-types.ts`, `resources.ts` e `icons.ts`. Nav, rutas, badges, paleta de categoría y validación de build derivan de ahí.
- Tres rutas dinámicas cubren el sitio (`[...slug]`, `tipos/[type]`, `categories/[category]`), más `/`, `/search`, `/tags`, 404, sitemap, robots, manifest y `search-index.json`.
- El build valida estructura de carpetas, relaciones rotas y enlaces internos muertos, y falla con mensaje en español.
- Búsqueda cliente con Fuse.js sobre un índice generado en build, descargado una vez por sesión.
...
- Contenido real: 718 entradas Markdown propias en `src/content/`.
